...
print("\x1b[1;97m┃╔═❲  LAGI LOADING.. ❳═>NanggXD  +18304338143")
print("\x1b[1;97m┃╚═════════════════════════════════")
print("\x1b[1;97m┣━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
print("\x1b[1;97m┃╔═❲ List Harga Api Key Tools️ ❳═>")
print("\x1b[1;97m┃║• Api Key Mingguan : 30K ")
print("\x1b[1;97m️┃║• Api Key 1 Bulan : 50K ")
print("\x1b[1;97m┃║• Api Key 2 Bulan : 100K ")
...
Password = "FARREL" # --> Data Req Api key --> open_kt(life)("AKUSAYANGNISA")
...
╭━━━┳━━━┳━━━┳━╮╭━┳━━┳╮╱╭┳━╮╭━╮╭━━┳━╮╱╭┳━━━┳━━━━┳━━━╮
┃╭━╮┃╭━╮┃╭━━┫┃╰╯┃┣┫┣┫┃╱┃┃┃╰╯┃┃╰┫┣┫┃╰╮┃┃╭━╮┃╭╮╭╮┃╭━╮┃
┃╰━╯┃╰━╯┃╰━━┫╭╮╭╮┃┃┃┃┃╱┃┃╭╮╭╮┃╱┃┃┃╭╮╰╯┃╰━━╋╯┃┃╰┫┃╱┃┃   AUTHOR : NANGG AKATSUKI
┃╭━━┫╭╮╭┫╭━━┫┃┃┃┃┃┃┃┃┃╱┃┃┃┃┃┃┃╱┃┃┃┃╰╮┃┣━━╮┃╱┃┃╱┃╰━╯┃   VERSION 4.4
┃┃╱╱┃┃┃╰┫╰━━┫┃┃┃┃┣┫┣┫╰━╯┃┃┃┃┃┃╭┫┣┫┃╱┃┃┃╰━╯┃╱┃┃╱┃╭━╮┃
╰╯╱╱╰╯╰━┻━━━┻╯╰╯╰┻━━┻━━━┻╯╰╯╰╯╰━━┻╯╱╰━┻━━━╯╱╰╯╱╰╯╱╰╯   ''',width=80,padding=(0,2),style=f"{color_table}"))
...
class instagram:
	...
	def hapus_lisensi(self):
		...
		if ask in ["y","Y"]:os.system("rm -rf data/lisensi.txt");prints(Panel(f"{P2}succeed menghapus {color_rich}'lisensi'{P2} terima kasih telah menggunakan script NanggXD",width=80,style=f"{color_table}"));time.sleep(2);exit()
		elif ask in ["t","T"]:self.menu()
		else:self.hapus_lisensi()

	def Exit(self):
		x=input(f" apakah anda yakin ingin keluar? Y/t : ")
		if x in ["y","Y"]:os.system("rm -rf data/.kukis.log rm -rf data/.username");prints(Panel(f"{P2}succeed menghapus {color_rich}'cookie' {P2}terima kasih telah menggunakan script NanggXD",width=80,padding=(0,2),style=f"{color_table}"));time.sleep(2);exit()
	...
	def useragent(self):
		...
		useragent = f'''Mozilla/5.0 (Linux; Android 10; SM-M205F Build/QP1A.190711.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/113.0.5672.131 Mobile Safari/537.36'''
		return useragent
...
def tlisensi():
    ...
    if lisen in ['']:
     ...
     tlisensi()
    if lisen in ['buy','Buy']:
     os.system('xdg-open https://wa.me/62895414128194?text=Bang+beli+lisensi+IG+nya+dong');exit()
    loadinglisen()
...
def key():
	...
	try:ses = requests.Session();asu = ses.get("https://app.cryptolens.io/api/key/Activate?token=WyIyNTYzMTUwMyIsIjlJNmI2M3lTcGtackhEaCtyc2JTLzlBZWZLdzZLYTJyZU4rNDNBZk4iXQ==&ProductId=16754&Key=%s&Sign=True"%(key)).json()['licenseKey']['key'];open("data/lisensi.txt","w").write(key);prints(Panel(f"{P2}selamat lisensi kamu aktif, tolong gunakan tools ini dengan bijak",width=80,padding=(0,4),style=f"{color_table}"));time.sleep(4);login_kamu()
		...
		prints(Panel(f"{P2} lisensi yang anda masukan tidak terdaftar silahkan beli terlebih dahulu",width=80,padding=(0,1),style=f"{color_table}"));os.system("xdg-open https://wa.me/+62895414128194?text=assalamualaikum+bang+mau+beli+lisensi+crack+Instagram");time.sleep(2);exit()
...
def cek():
	...
	try:x = requests.get("https://app.cryptolens.io/api/key/Activate?token=WyIyNTYzMTUwMyIsIjlJNmI2M3lTcGtackhEaCtyc2JTLzlBZWZLdzZLYTJyZU4rNDNBZk4iXQ==&ProductId=16754&Key=%s"%(x)).json()['licenseKey']['key'];login_kamu()
		...
		prints(Panel(f"{P2}lisensi kamu sudah kedaluwarsa silahkan beli lisensi ke admin",width=80,padding=(0,6),style=f"{color_table}"));os.system("rm -rf data/lisensi.txt");os.system("xdg-open https://wa.me/+62895414128194?text=assalamualaikum+bang+mau+beli+lisensi+crack+Instagram");time.sleep(2);exit()
...
def beli_bang():
    ...
    elif zxc in ["1","01"]:os.system("xdg-open https://wa.me/+62895414128194?text=assalamualaikum+bang+mau+beli+lisensi+1+minggu");time.sleep(2);exit()
    elif zxc in ["2","02"]:os.system("xdg-open https://wa.me/+62895414128194?text=assalamualaikum+bang+mau+beli+lisensi+1+bulan");time.sleep(2);exit()
    elif zxc in ["3","03"]:os.system("xdg-open https://wa.me/+62895414128194?text=assalamualaikum+bang+mau+beli+lisensi+2+bulan");time.sleep(2);exit()
    elif zxc in ["4","04"]:os.system("xdg-open https://wa.me/+62895414128194?text=assalamualaikum+bang+mau+beli+lisensi+permanen");time.sleep(2);exit()
